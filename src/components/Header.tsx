...
export default function Header({ onScrollTo }: HeaderProps) {
  ...
  return (
    ...
    >
      ...
      {/* Navigation */}
      <div className="flex-1 mx-2 sm:mx-4 overflow-x-auto scrollbar-hide">
        <div className="flex items-center gap-1 sm:gap-2 bg-[#EBE7DF]/50 py-1 px-2 sm:px-3 rounded-none border border-on-surface/30 w-max mx-auto">
          {menuItems.map((item) => (
            <button
              key={item.id}
      ...
      <div className="flex-shrink-0">
        <button
          ...
          className="px-3 sm:px-4 py-1.5 bg-primary text-on-primary font-mono text-[10px] sm:text-xs uppercase tracking-[0.2em] font-bold rounded-none hover:bg-neutral-800 transition-all border border-primary cursor-pointer whitespace-nowrap"
        >
          Hire
        </button>
      </div>
